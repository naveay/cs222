...
int RBFTest_8(RecordBasedFileManager *rbfm) {
    ...
    prepareRecord(7, "Peters2", 24, 170.1, 5000, record, &recordSize);
    rbfm->updateRecord(fileHandle, recordDescriptor, record, rid);
    rc = rbfm->readRecord(fileHandle, recordDescriptor, rid, returnedData);
    rbfm->readAttribute(fileHandle, recordDescriptor, rid, "Salary",returnedData);
    int i;
    memcpy(&i,returnedData,sizeof(int));
    cout<<i;
    //rbfm->printRecord(recordDescriptor, returnedData);
